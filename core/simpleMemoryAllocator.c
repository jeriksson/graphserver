...
void memFreeObjectsAndResources(simpleMemoryAllocator * this, void (*f)(void *, void *, int)) {
	int i = 1; 
	for (; i < this->currentObjectsAllocated; ++i)
	{
		(*f)(this->objects + (i * this->sizeOfType), this->objects, i);
	}

	(*f)(this->objects, this->objects, i);
	free(this->objects);
	free(this);
}
